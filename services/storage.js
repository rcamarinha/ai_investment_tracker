        ...
        for (const [symbol, txs] of Object.entries(state.transactions)) {
            for (const tx of txs) {
                rows.push({
                    ...
                    // coerceTxDate ensures the DATE NOT NULL constraint is always met
                    // (an AI-extracted row could miss a clean date otherwise, which
                    // would abort the whole bulk insert).
                    date: coerceTxDate(tx),
