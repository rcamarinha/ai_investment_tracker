...
- **`?v=` on `services/` imports** — `vercel.json` serves `/services/(.*)` with `max-age=31536000, immutable`, so a *relative* import of a pure core (`from './spend-core.js'`) is frozen for a year and no bugfix ever reaches a returning user. Import pure services **with** `?v=`. (`/src/` is *not* in the immutable list, which is why the older `wine/ → ../src/wine.js` imports are safe without one.) Several pre-existing `services/ → services/` imports still have this problem.
