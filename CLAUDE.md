...
- **A personal choice must never live in a shared table** — `assets` is a catalogue every account may write, and it carried `untracked` ("keep at cost") and `pricing_ticker` (the learned symbol `services/pricing.js` actually queries). One person's choice therefore stopped or repointed pricing for EVERY holder of that ticker, and a repointed ticker put a plausible wrong figure into their snapshots and the hub's net worth. Migration `20260921_per_user_asset_prefs.sql` moves both to `user_asset_prefs` (owner-scoped), backfilled to every current holder so nobody's behaviour changed, and an `assets_guard` trigger freezes those two columns for browser roles and clamps what they may write (it CLAMPS rather than raises: `saveAssetsToDB` upserts a batch and only warns, so a raise would silently lose the batch). The trigger lets the service role and the SQL editor through, so a bad value stays repairable. `loadAssetPrefs()`/`saveAssetPref()` in storage.js are the only readers and writers; `buildAssetRecord` no longer carries `untracked`.
